...
%
%------------------------------------------------------------
function Vnet_sd_ave_screen=EM_mainModule_hypothetical_base(x1,x2)


clc
delete MyPerturbation.mat MyPerturbationN.mat%Ali
load GlycVI_myPTS_ru5p_noHXK_reg.mat

% Reformat the model provided by Jimmy to be consistent with the codes written by Ali
model=reformatModel(Net,[0 32],[29 30 31],'GlycVI_myPTS_ru5p_noHXK_reg');%old version%model=reformatModel(Net,1,'GlycVI_myPTS_ru5p_noHXK');
...
while done == 0
  ...
  for  n=1:(ensemble_size - acceptModel_counter)
     ...
     % Create the model
     [kinetic_param,~,~,sampled_R_vec,~,sampled_e_vec,MyRev] = createModel(expanded_model);
     ...
     sampled_R_vec_ens_try(:,n)=sampled_R_vec;
     sampled_Rev_vec_ens_try(:,n)=MyRev;
  ...
  for  n=1:(ensemble_size - acceptModel_counter)
     if  abs(Vnet_ode_ens_try(export_indices(1),end,n)  - expanded_model.unexp_rxn_info{export_indices(1),7}) < Vnet_thr*expanded_model.unexp_rxn_info{export_indices(1),7}
       ...
       sampled_R_vec_ens(:,acceptModel_counter) = sampled_R_vec_ens_try(:,n);
...
% save(strcat('ensemble_',expanded_model.model_name,'.mat'),'expanded_model','kinetic_param_ens','sampled_e_vec_ens','kinetic_param_base','sampled_e_vec_base','sampled_R_vec_ens'); 
load ensemble_GlycVI_myPTS_ru5p_noHXK_reg.mat
% ensemble_GlycVI_myPTS_ru5p_noHXK.mat=ensemble_GlycVI_myPTS_ru5p_noHXK_reg.mat;
% strcat('ensemble_',expanded_model.model_name,'.mat')=ensemble_GlycVI_myPTS_ru5p_noHXK_reg.mat
%-------------------------------------------------------------
...
%+++++++++++++++++
% uptake_indices1 = find(cell2mat(expanded_model.unexp_rxn_info(:,6)) == 4);
% uptake_indices2 = find(cell2mat(expanded_model.unexp_rxn_info(:,6)) == 2);
% uptake_indices=sort([uptake_indices1;uptake_indices2]);
uptake_indices=[];
screen_rxn_indices=[32];%[32 35 36 37];%strmatch('pyk', char(expanded_model.Enzyme));
...
%    Loe=1;
   ...
   for i=1:length(x1)
       expanded_model.Perturbations(x1(i),2)=x2(i);
   end
   
%    expanded_model.Perturbations(5,2)=2;%Ali---------------------------------
%    if n_perturbType == 1
...
%    end
   ...
   if size(expanded_model.Perturbations,2) > 1
    ...
    for np = 2:size(expanded_model.Perturbations,2)
       ...
       %fprintf('%s for the base model ...\n\n',expanded_model.perturb_names{np});%Ali+++++++++++++++
       ...
       [metrics_perturb,Vnet_perturb_ode_ens,conc_perturb_ode_ens] = perturbEnsemble(expanded_model,kinetic_param_current_ens,sampled_e_vec_current_ens,[uptake_indices;screen_rxn_indices],t_interval);
...
%        end
       ...
       MyperturbationN{flag2,1}=(expanded_model.perturb_names(2));
       Myperturbation(flag1,:)=metrics_perturb';

       
% Vnet_perturb_ss_ode_ens_ali(:,:) = Vnet_perturb_ode_ens(:,end,:);
% [mm,nn]=size(Vnet_perturb_ss_ode_ens_ali);
% for i=nn
%     for j=nn
%         pp(i,j,n_perturbType,n_enz) = ranksum(Vnet_perturb_ss_ode_ens_ali(:,i),Vnet_perturb_ss_ode_ens_ali(:,j));
%         hh(i,j,n_perturbType,n_enz) = kstest2(Vnet_perturb_ss_ode_ens_ali(:,i),Vnet_perturb_ss_ode_ens_ali(:,j));
%         
%     end
% end
       
       %fprintf('# accepted = %i , # rejected = i\n',acceptModel_counter,current_ensemble_size - acceptModel_counter);%Ali+++++++++++++++++
...
% end %for if perturb%Ali





% [p,a,s] = anova1(Vnet_perturb_ss_ode_ens_ali);
% [c,m,h,nms] = multcompare(s);
% p = kruskalwallis(Vnet_perturb_ss_ode_ens_ali);

%save('MyPerturbation.mat','Myperturbation','MyperturbationN','sampled_Rev_vec_ens_try');%,'pp','hh'
% Save the perturbation results in a mat file
